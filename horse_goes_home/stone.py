...
def showEndEvent(RESULT,cx,cityx):
	...
	drumtime = 0
	DRUMLEN = (DRUM.get_length()-4)*1000 # millisecond
	...
	score = 85.000
	while True:
		if drumtime+DRUMLEN-pygame.time.get_ticks()<0:
...
def showStartScreen():
    ...
    CHOICE.set_volume(0.2)
    pygame.mixer.music.load("se/13096.wav")
    ...
    while True:
        ...
        DISPLAYSURF.blit(IMAGEDICT['title'],(0,titley-10))
        ...
        horsey = 6*math.sin(pygame.time.get_ticks()/300)+noise
        horsex += speed
        if horsex % 40 == 0:
        	if speed > 0:
       			if im == 'crun1R':
...
def drawPressKeyMsg2():
    pressKeySurf = BASICFONT.render('Press a key to go to the Title.', True, DARKGRAY)
    pressKeyRect = pressKeySurf.get_rect()
    pressKeyRect.topleft = (WIDTH - 255, HEIGHT - 30)
    DISPLAYSURF.blit(pressKeySurf, pressKeyRect)
